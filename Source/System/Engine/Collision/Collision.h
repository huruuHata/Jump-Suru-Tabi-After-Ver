...
public:
	...
	// 戻り値：bool 衝突している場合 true、していない場合 false
	...
	// 　　　　const D3DXVECTOR3[3] ポリゴンの頂点配列
	// 　　　　D3DXVECTOR3*         交点があった場合、交点の座標を格納する変数へのポインタ
	//         const bool           レイの始点を無視する（地形データなど例外的に使用）
	// 備　考：表ポリゴンのみ判定を行う。
	//=============================================================================
	static bool CheckCrossPoint(const D3DXVECTOR3* pVecPos, const D3DXVECTOR3* pVecRay, const D3DXVECTOR3* pVecNormal, const D3DXVECTOR3 vPolygon[3], D3DXVECTOR3* pVecCrossPoint, const bool bIgnoreStartPos = false);
