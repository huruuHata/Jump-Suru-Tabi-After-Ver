...
public:
	...
	enum OriginalMessage {
		...
		OM_BGM_CREATE_BASICAUDIO_ERROR,          //＄ベーシックオーディオの作成に失敗
		...
		OM_CREATE_WINDOW_HEIGHT_ERROR,           //スクリーンの高さを超えるウインドウは作成できません
		OM_GET_RENDERTARGET_ERROR,               //レンダリングターゲットの取得に失敗
		OM_GET_DEPTHSTENCIL_ERROR,               //深度バッファの取得に失敗
		OM_SET_RENDERTARGET_ERROR,               //レンダリングターゲットの設定に失敗
		OM_SET_DEPTHSTENCIL_ERROR,               //深度バッファの設定に失敗
		OM_CREATE_DEPTHSTENCIL_ERROR,            //深度バッファの作成に失敗
		OM_GET_SURFACE_ERROR,                    //サーフェスの取得に失敗
		OM_SET_TECHNIQUE_ERROR,                  //テクニックの設定に失敗
	};
