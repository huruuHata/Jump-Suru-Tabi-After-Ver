...
// フレーム行列の更新（親子関係の設定）
// 引　数：LPD3DXFRAME        フレームへのポインタ
// 　　　　const LPD3DXMATRIX 親フレームのマトリックス
//=============================================================================
void Frame::UpdateFrameMatrices(LPD3DXFRAME pFrameBase, const LPD3DXMATRIX pParentMatrix)
{
...
//=============================================================================
void Frame::DrawFrame(LPD3DXFRAME pFrame, const LPD3DXMATRIX pMatVP, ID3DXEffect* pEffect)
{
...
//=============================================================================
// ルートフレームのポインタのアドレス取得
// 戻り値：LPD3DXFRAME* ルートフレームのポインタのアドレス
//=============================================================================
...
// 戻り値：LPD3DXMESH ルートフレーム内のメッシュへのポインタ
//=============================================================================
...
//         ID3DXEffect*        エフェクトへのポインタ
//-----------------------------------------------------------------------------
void Frame::DrawMeshContainer(LPD3DXMESHCONTAINER pMeshContainerBase, LPD3DXFRAME pFrameBase, const LPD3DXMATRIX pMatVP, ID3DXEffect* pEffect)
{
	D3DXMESHCONTAINER_DERIVED* pMeshContainer = (D3DXMESHCONTAINER_DERIVED*)pMeshContainerBase;
	D3DXFRAME_DERIVED* pFrame = (D3DXFRAME_DERIVED*)pFrameBase;
	...
	if (pMeshContainer->pSkinInfo) {
		...
		for (UINT subset = 0; subset < pMeshContainer->numBoneCombinations; subset++) {
			...
			for (int i = 0; i < 4; i++) {
				matRotate[i] = matWorld[i];
				matRotate[i].m[3][0] = 0.0f;
				matRotate[i].m[3][1] = 0.0f;
				matRotate[i].m[3][2] = 0.0f;

				D3DXMatrixInverse(&matRotate[i], nullptr, &matRotate[i]);
				D3DXMatrixTranspose(&matRotate[i], &matRotate[i]);
			}

			//ボーンの変換行列を送る
			pEffect->SetMatrixArray("gMatW", matWorld, 4);
			pEffect->SetMatrixArray("gMatR", matRotate, 4);
		...
		//ワールド転置逆行列を渡す
		D3DXMATRIX matWIT = matWorld;
		matWIT.m[3][0] = 0.0f;
		matWIT.m[3][1] = 0.0f;
		matWIT.m[3][2] = 0.0f;
		D3DXMatrixInverse(&matWIT, nullptr, &matWIT);
		D3DXMatrixTranspose(&matWIT, &matWIT);
		pEffect->SetMatrix("gMatWIT", &matWIT);

		//ワールドビュー射影変換行列を渡す
		D3DXMATRIX matWVP = matWorld * *pMatVP;
