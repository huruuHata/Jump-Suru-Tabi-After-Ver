...
public:
	...
	// 　　　　const unsigned int 方向（DIK_HORIZONTAL, DIK_VERTICAL）
	//=============================================================================
	int GetPadCrossKeyStateSync(const unsigned int idxPad, const unsigned int direction);
...
private:
	...
	//-----------------------------------------------------------------------------
	// デバイス作成（コールバック関数）
	// 戻り値：BOOL DIENUM_STOP 列挙の停止、DIENUM_CONTINUE 列挙の継続
	// 引　数：const LPDIDEVICEINSTANCE 入力デバイス構造体のポインタ
	// 　　　　VOID*                    InputManageクラスのポインタ
	//-----------------------------------------------------------------------------
	friend static BOOL CALLBACK EnumGamepadCallback(const LPDIDEVICEINSTANCE pdidInstance, VOID *pContext);

	//-----------------------------------------------------------------------------
	// 軸の値の範囲設定（コールバック関数）
	// 戻り値：BOOL DIENUM_STOP 列挙の停止、DIENUM_CONTINUE 列挙の継続
	// 引　数：const DIDEVICEOBJECTINSTANCE 入力デバイス構造体のポインタ
	// 　　　　VOID*                        GamePad構造体のポインタ
	//-----------------------------------------------------------------------------
