...
//            ＢＧＭの音量、パンを調整する関数を追加
//            ＳＥの周波数、音量、パンを調整する関数名を変更
//
// 2023/11/07 ＢＧＭ再生中チェック関数を追加
//
// 2024/11/25 PlaySE関数にループフラグを追加
//
//*****************************************************************************
...
void Engine::Initialize(const HWND& hAppWnd, const HINSTANCE& hInstance)
{
	...
	//ウインドウハンドル
	d3dparam.hDeviceWindow = m_hAppWnd;
	...
	//バックバッファのサイズ
	d3dparam.BackBufferWidth = WindowSetting::WINDOW_WIDTH;
	d3dparam.BackBufferHeight = WindowSetting::WINDOW_HEIGHT;
	...
	//デバイスの作成
	hResult = m_pDirect3D9->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, m_hAppWnd, behaviorFlag, &d3dparam, &m_pDevice);
...
//=============================================================================
// 画面のクリア
// 引　数：const D3DCOLOR 背景色
//=============================================================================
void Engine::Clear(const D3DCOLOR backgroundColor)
{
...
// 引　数：const RECT*  転送先情報の入ったRECT構造体へのポインタ
// 　　　　string       転送する画像のファイル名
// 　　　　const RECT*  転送元情報の入ったRECT構造体へのポインタ
// 　　　　const DWORD  アルファ値(０から２５５まで)
// 　　　　const float  回転角度(ラジアン角)
//=============================================================================
...
//=============================================================================
// 指定領域への文字転送
// 引　数：const RECT*    表示領域を示すRECT構造体へのポインタ
// 　　　　string         使用するフォント名
// 　　　　const D3DCOLOR 文字色
// 　　　　string         表示文字
//=============================================================================
...
//=============================================================================
// 数字画像を描画
// 引　数：const int   描画位置のx座標
// 　　　　const int   描画位置のy座標
// 　　　　const DWORD 描画したい数値（正の整数のみ）
// 　　　　const int　 表示したい桁数
// 　　　　const bool　前ゼロ制御（true 前ゼロあり、false なし）
// 　　　　const int　 転送元のx座標（画像の'０'の左上座標）
// 　　　　const int　 転送元のy座標（画像の'０'の左上座標）
// 　　　　const int　 数字画像１つ分の幅
// 　　　　const int　 数字画像１つ分の高さ
// 　　　　string      画像のファイル名
...
// 2D用テクスチャの追加
// 引　数：string         画像のファイル名
// 　　　　const D3DCOLOR カラーキー（よく使うものはTexture.hに定数化してある）
//=============================================================================
void Engine::AddTexture(std::string strFileName, const D3DCOLOR colorKey)
{
	assert(strFileName.length() > 0);
...
void Engine::AddFont(std::string strSizeAndFontName)
{
	...
	//登録済みならreturn
	if (m_pClsFontManage->SearchFont(strSizeAndFontName)) {
		return;
	}
...
// 引　数：string フォントサイズ＋フォント名（例 "40:ＭＳ ゴシック" ）
//=============================================================================
int Engine::GetFontSize(std::string strFontName)
{
	return m_pClsFontManage->GetFontSize(strFontName);
}
...
// ボリュームをリセット
//=============================================================================
void Engine::ResetBGMVolume()
{
	m_pClsBGMManage->ResetBGMVolume();
}
...
//=============================================================================
void Engine::ChangeBGMVolume(const int value)
{
	m_pClsBGMManage->ChangeBGMVolume(value);
}

//=============================================================================
// パンをリセット
//=============================================================================
void Engine::ResetBGMPan()
{
	m_pClsBGMManage->ResetBGMPan();
}
...
// 引　数：const int 増減させる数値
//=============================================================================
void Engine::ChangeBGMPan(const int value)
{
	m_pClsBGMManage->ChangeBGMPan(value);
}
...
//=============================================================================
bool Engine::IsPlayingBGM()
{
	return m_pClsBGMManage->IsPlaying();
}
...
// 引　数：string ＢＧＭファイル名
//=============================================================================
bool Engine::IsPlayingBGM(std::string strFileName)
{
	return m_pClsBGMManage->IsPlaying(strFileName);
}
...
// ＳＥの再生
// 引　数：string      ＳＥファイル名
// 　　　：const DOWRD 優先度（0～0xffffffff）デフォルトは０
// 　　　：const bool  ループフラグ　デフォルトはfalse
//=============================================================================
void Engine::PlaySE(std::string strFileName, const DWORD priority, const bool bLoop)
{
	m_pClsSEManage->PlaySE(strFileName, priority, bLoop);
}
#endif
...
// 引　数：string ＳＥファイル名
//=============================================================================
void Engine::ResetSEFrequency(std::string strFileName)
{
	m_pClsSEManage->ResetFrequency(strFileName);
}
...
// 　　　　const int 増減させる周波数
//=============================================================================
void Engine::ChangeSEFrequency(std::string strFileName, const int value)
{
	m_pClsSEManage->ChangeFrequency(strFileName, value);
}
...
// 引　数：string ＳＥファイル名
//=============================================================================
void Engine::ResetSEPan(std::string strFileName)
{
	m_pClsSEManage->ResetPan(strFileName);
}
...
// 　　　　const int 増減させる数値
//=============================================================================
void Engine::ChangeSEPan(std::string strFileName, const int value)
{
	m_pClsSEManage->ChangePan(strFileName, value);
}
...
// 引　数：string ＳＥファイル名
//=============================================================================
void Engine::ResetSEVolume(std::string strFileName)
{
	m_pClsSEManage->ResetVolume(strFileName);
}
...
// 　　　　const int 増減させる数値
//=============================================================================
void Engine::ChangeSEVolume(std::string strFileName, const int value)
{
	m_pClsSEManage->ChangeVolume(strFileName, value);
}
#endif
...
// モデルデータの追加
// 引　数：string     Ｘファイル名
// 　　　：const bool 隣接データ作成フラグ（デフォルトはfalse）
// 備　考：追加するだけでは使用出来ない。GetModelを使って取得する必要がある。
//=============================================================================
void Engine::AddModel(std::string strFileName, const bool bCreateAdjacencyData)
{
...
//=============================================================================
// パッドが接続されているか確認する
// 戻り値：bool true 接続済み、false 接続されていない
//=============================================================================
...
void Engine::ScreenShot()
{
	...
	tm timeManage;
	timer = time(nullptr);
	localtime_s(&timeManage, &timer);

	//スクリーンショット用フォルダが無い場合は作成する
	if (!PathIsDirectory((LPCSTR)SCREENSHOT_FOLDER.c_str())) {
		CreateDirectory((LPCSTR)SCREENSHOT_FOLDER.c_str(), nullptr);
	}

	//"日付時間.bmp"の形にする
	ssFileName << SCREENSHOT_FOLDER << "\\"
		<< timeManage.tm_year + 1900
		<< std::setw(2) << std::setfill('0') << std::right << timeManage.tm_mon + 1
		<< std::setw(2) << std::setfill('0') << std::right << timeManage.tm_mday
		<< std::setw(2) << std::setfill('0') << std::right << timeManage.tm_hour
		<< std::setw(2) << std::setfill('0') << std::right << timeManage.tm_min
		<< std::setw(2) << std::setfill('0') << std::right << timeManage.tm_sec
		<< ".bmp";
	...
	//スクリーンショットの保存
	hResult = D3DXSaveSurfaceToFile((LPCSTR)ssFileName.str().c_str(), D3DXIFF_BMP, pSurface, nullptr, &rect);
...
//=============================================================================
// 直線の描画（２Ｄ）
// 引　数：const D3DXVECTOR2 開始位置（２Ｄベクトル）
// 　　　　const D3DXVECTOR2 終了位置（２Ｄベクトル）
// 　　　　const D3DCOLOR    線の色
//=============================================================================
void Engine::DrawLine(const D3DXVECTOR2 startPosition, const D3DXVECTOR2 endPosition, const D3DCOLOR color)
{
...
//=============================================================================
// 直線の描画（２Ｄ）
// 引　数：const int      開始Ｘ座標、Ｙ座標
// 　　　　const int      終了Ｘ座標、Ｙ座標
// 　　　　const D3DCOLOR 線の色
//=============================================================================
void Engine::DrawLine(const int startX, const int startY, const int endX, const int endY, const D3DCOLOR color)
{
...
//=============================================================================
void Engine::DrawLine(const D3DXVECTOR3 startPosition, const D3DXVECTOR3 endPosition, const D3DCOLOR color, const D3DXMATRIX* pView, const D3DXMATRIX* pProjection)
{
...
//=============================================================================
// 円の描画
// 引　数：const D3DXVECTOR2 中心座標（２Ｄベクトル）
// 　　　　const float       半径（スクリーン座標）
// 　　　　const int         分割数（３６０が割り切れる値）
// 　　　　const D3DCOLOR    線の色
//=============================================================================
...
// 引　数：ImVec2           位置（左上の座標）
// 　　　　ImVec2           サイズ（幅、高さ）
// 　　　　ImGuiWindowFlags フラグ（デフォルトはタイトルなし、リサイズなし、移動なし）
//=============================================================================
...
//=============================================================================
// バックバッファを取得
// 戻り値：LPDIRECT3DSURFACE9 バックバッファのポインタ
//=============================================================================
