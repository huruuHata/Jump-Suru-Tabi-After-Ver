...
//
// 2023/02/10 スクリーンの高さを超えるウインドウは作れないコードを追記
//
//*****************************************************************************
...
void Window::Initialize(const HINSTANCE hInstance, const int nShowCmd)
{
	...
	//Windowスタイルの決定
	if (MODE_WINDOW) {
		m_windowStyle = WS_CAPTION | WS_SYSMENU;
	}
	else {
	...
	RECT clientRect;
	GetClientRect(m_hAppWnd, &clientRect);

	if (clientRect.bottom < WINDOW_HEIGHT) {
		throw DxSystemException(DxSystemException::OM_CREATE_WINDOW_HEIGHT_ERROR);
	}
}
