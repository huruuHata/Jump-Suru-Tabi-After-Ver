...
VS_OUTPUT TextureVS_PB1NT(VS_INPUT_PB1NT In)
{
	...
	float4x4 combW = (float4x4)0;
	float4x4 combR = (float4x4)0;

	//合成行列にブレンド値を加えワールド座標変換行列を作成
	int i;
	for (i = 0; i < gNumMaxInfle - 1; i++) {
		combW += gMatW[i] * In.blend;
		combR += gMatR[i] * In.blend;
	}
	combW += gMatW[i] * (1.0f - In.blend);
	combR += gMatR[i] * (1.0f - In.blend);

	//頂点を調整
	Out.position = mul(float4(In.position, 1.0f), combW);
	Out.position = mul(Out.position, gMatVP);

	//法線を調整
	float4 normal = normalize(mul(float4(In.normal, 0.0f), combR));
...
VS_OUTPUT TextureVS_PB3NT(VS_INPUT_PB3NT In)
{
	...
	int i;
	for (i = 0; i < gNumMaxInfle - 1; i++) {
		combW += gMatW[i] * blend[i];
		combR += gMatR[i] * blend[i];
	}
	combW += gMatW[i] * (1.0f - blend[0] - blend[1] - blend[2]);
	combR += gMatR[i] * (1.0f - blend[0] - blend[1] - blend[2]);
...
VS_OUTPUT TextureVS_PB2NCT(VS_INPUT_PB2NCT In)
{
	...
	int i;
	for (i = 0; i < gNumMaxInfle - 1; i++) {
		combW += gMatW[i] * In.blend[i];
		combR += gMatR[i] * In.blend[i];
	}
	combW += gMatW[i] * (1.0f - In.blend[0] - In.blend[1]);
	combR += gMatR[i] * (1.0f - In.blend[0] - In.blend[1]);
...
VS_OUTPUT TextureVS_PB3NCT(VS_INPUT_PB3NCT In)
{
	...
	combW += gMatW[i] * (1.0f - In.blend[0] - In.blend[1] - In.blend[2]);
	combR += gMatR[i] * (1.0f - In.blend[0] - In.blend[1] - In.blend[2]);
